...
=======
        ...
        public async Task<IActionResult> CerrarSesion()
        {
            try
            {
                var usuarioEmail = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;

                if (string.IsNullOrEmpty(usuarioEmail))
                    return Unauthorized(new { mensaje = "Usuario no autenticado" });

                var sesion = await _context.tSesiones.FirstOrDefaultAsync(x => x.SUsuario == usuarioEmail);

                if (sesion == null)
                    return NotFound(new { mensaje = "No hay sesión activa" });
            ...
            {
                return StatusCode(500, new { mensaje = "Error al cerrar sesión", detalle = ex.Message });
            }
>>>>>>> edbb295 (Subiendo mi proyecto a Miguel)
        }
